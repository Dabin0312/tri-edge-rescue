...
**Tri-Edge Rescue**는 재난·실험실 사고 환경을 가정한 온디바이스 멀티로봇 탐색·구조 AI 시스템입니다.
...
Tri-Edge Rescue는 두 대의 로봇이 각자 현장에서 로컬 정보를 수집하고, 원본 영상이나 대용량 센서 데이터를 중앙으로 보내지 않고, 의미 정보인 `summary JSON`만 Commander C에 전달하는 구조를 구현합니다.

Commander C는 Robot A/B의 summary를 통합해 위험도와 상황을 판단하고, 각 로봇에게 `task command`를 다시 전송합니다.

---

## 2. Core Concept

기존 클라우드 기반 구조에서는 로봇의 카메라 영상이나 센서 데이터를 외부 서버로 전송해야 하므로 다음 문제가 발생할 수 있습니다.
...
Tri-Edge Rescue는 각 로봇이 로컬에서 의미 정보를 생성하고, 중앙 Commander는 이 summary만 받아 판단합니다.
...
Robot A/B는 원본 영상이나 전체 센서 스트림 대신 다음과 같은 summary JSON만 Commander C에 전송합니다.
...
Commander C는 summary를 바탕으로 각 로봇에게 명령을 다시 전송합니다.
...
Gazebo world는 재난·실험실 사고 환경을 단순화해 구성합니다.
...
최종적으로 Jetson Orin Nano 3대는 다음과 같이 배치합니다.
